...
import { CartsToolbar } from "./carts-toolbar";

interface CartsTableProps<TData, TValue> {
  columns: ColumnDef<TData, TValue>[];
  data: TData[];
  client?: boolean;
}

export function CartsTable<TData, TValue>({
  columns,
  data,
  client,
}: CartsTableProps<TData, TValue>) {
  ...
  const handleRefetch = async () => {
    setRefetching(true);
    utils.carts.getCarts.invalidate();
    setTimeout(() => {
      setRefetching(false);
    }, 1000);
  };
  ...
  const userStatuses = [
    { label: statusLabels["dropped"], value: "DROPPED" },
    // { label: statusLabels["abandonned"], value: "ABANDONNED" },
    { label: statusLabels["waiting_payment"], value: "WAITING_PAYMENT" },
    { label: statusLabels["paid"], value: "PAYED" },
    // { label: statusLabels["forgotten"], value: "FORGOTTEN" },
  ];
  const adminStatuses = [
    { label: statusLabels["dropped"], value: "DROPPED" },
    { label: statusLabels["abandonned"], value: "ABANDONNED" },
